...
const getDocument = (asyncapi) => asyncapi?._json || asyncapi || {};
...
const getOperations = (doc) => {
  ...
  Object.entries(componentOperations).forEach(([key, operation]) => {
    ...
    ops.push({ ...operation, key, name: operation.name || operation.operationId || key, action });
  });
...
const buildSendMethods = (operations) => operations
  .filter((operation) => operation.action === 'send' || operation.action === 'request')
  .map((operation) => {
...
const buildReceiveHandlerMethods = (operations) => operations
  .filter((operation) => operation.action === 'receive' || operation.action === 'reply')
  .map((operation) => {
...
export default function ClientRs({ asyncapi }) {
